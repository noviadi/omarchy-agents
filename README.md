...
| **Tab order** | `Main.qml` + manifest | Stock order is alphabetical. `providerOrder` (default `["zai", "claude", "codex", "fireworks"]`) puts listed ids first — the first entry is the tab the panel opens on. Override with `omarchy bar set "$(id -un).agents" providerOrder '["claude","zai"]' --json`. |
